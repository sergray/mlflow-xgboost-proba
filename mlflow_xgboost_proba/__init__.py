"""
The ``mlflow_xgboost_proba`` module provides an API for logging and loading XGBoost models.

It is modification of ``mlflow.xgboost`` module to support the probabilities in predictions using ``mlflow models serve``.

Original ``mlflow.xgboost`` module can be found at https://github.com/mlflow/mlflow/blob/master/mlflow/xgboost/__init__.py
"""
...
def get_default_pip_requirements():
    ...
    """
    return [
        _get_pinned_requirement("xgboost"),
        _get_pinned_requirement("mlflow_xgboost_proba"),
    ]
...
class XGModelProbaWrapper(mlflow.xgboost._XGBModelWrapper):
    """
    Wrapper around MLflow XGBoost model that supports predict_proba method via parameters.
    """
    ...
    def predict_proba(
        ...
        if isinstance(self.xgb_model, xgb.Booster):
            ...
            return self.xgb_model.predict_proba(dataframe)
    ...
    def predict(
        ...
        with_probabilities = params.pop("with_proba", True)

        if not with_probabilities:
            return super().predict(dataframe, params=params)

        return self.predict_proba(dataframe, params=params)
...
def _load_pyfunc(path):
    ...
    Args:
        path: Local filesystem path to the MLflow Model with the ``xgboost`` flavor.
    """
    return XGModelProbaWrapper(_load_model(path))
...
def save_model(  # noqa: C901
    ...
    if signature is None and input_example is not None:
        wrapped_model = XGModelProbaWrapper(xgb_model)
    ...
    pyfunc.add_to_model(
        mlflow_model,
        loader_module="mlflow_xgboost_proba",
